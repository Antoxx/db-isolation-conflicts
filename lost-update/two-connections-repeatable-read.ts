...
await Promise.all([
  increaseBalance(client1, 1.01, 0, ISOLATION_LEVEL.REPEATABLE_READ),
  increaseBalance(client2, 10.1, 0, ISOLATION_LEVEL.REPEATABLE_READ),
])

balance = await logBalance(client1) // 101.01 - the first is a winner with REPEATABLE_READ, the second failed with error
...
balance = await logBalance(client1) // 110.1 - the second is a winner with REPEATABLE_READ (started later but finished fast), the first failed with error
